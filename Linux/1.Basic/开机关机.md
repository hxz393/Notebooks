...
虚拟文件系统的img文件也能被boot loader载入到内存中,解压后在内存中仿真成一个根目录.内核会读取虚拟文件系统内的驱动,通常是与磁盘接口的驱动程序.等驱动载入完毕,此虚拟文件系统被卸载,并挂载实际的根目录文件系统,继续开始后续开机流程.
