...
统计重复数据可以使用uniq命令,例如使用-c参数统计用户登录次数再以从少到多排序:
...
比较纯文本文件可以用diff命令.同时也能够通过diff创建的分析文件,以处理补丁功能的文件.
...
可以直接将文件比较的区别制作成补丁文件,供要比较的文件使用升级.制作补丁参数为**-Naur**:
...
相对于diff以行为单位比较,cmp则是以字节为单位去比较.例如还是比较c.cfg 与cnew.cfg:
