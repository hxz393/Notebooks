...
Shell是一个命令行解释器,其功能是提供用户操作系统的一个接口,因此shell需要可以调用其他软件,用户通过shell来操作这些应用程序.也就是只要能够操作应用程序的接口都能够称为shell.
...
操作符>有几个修饰符标记,这些标记可以改变它的行为.例如:

1. 操作符>&表示将标准输出和错误同时进行重定向;
2. 操作符>!将迫使文件以append模式创建,或者以normal模式覆盖一个已存在的文件;
3. 操作符>@将以二进制模式而不是文件模式打开一个文件.
