...
```sh
[root@server6-node2 ~]# echo "127.0.0.1   $(hostname)" >> /etc/hosts
```
...
关闭防火墙,同时将SELinux设置为被动模式,否则容器访问主机文件系统会不正常:
...
使用指定参数初始化.service-cidr与pod-network-cidr使用的网段对应flannel网络插件.如果是Calico网络插件默认使用pod-network-cidr=192.168.0.0/16:
...
如果kubeadm初始化时没设置--pod-network-cidr=192.168.0.0/16,需要修改yaml文件中CIDR值:
...
可以使用kubectl delete node来删除节点:

```sh
[root@server4-master ~]# kubectl delete node server3
